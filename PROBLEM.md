...
Working name during development: `woo-stripe-reconcile`. Renamed at WP.org rollout planning (2026-08-30, phase 1) to **Stripe Order Reconciler for WooCommerce** to stop leading with the WooCommerce trademark — but that rename still led with "Stripe," which WordPress.org's own published naming guidance rejects on the same "can't lead with a trademark you don't own" basis (their guidelines list "Stripe Payments" as a disallowed example for exactly this reason). Corrected 2026-09-12 (second pass) to **Payment Order Reconciler for WooCommerce and Stripe**, reasoning both trademarks were now merely trailing, not leading — but the actual WP.org plugin review team rejected this submission outright: their policy on "woocommerce" specifically is a flat ban from the name/slug in any position, not the general "don't lead" rule that applies to most trademarks, and they explicitly warn against letter-substitution workarounds. Corrected 2026-09-12 (third pass, in response to that rejection): public name **Payment Order Reconciler for Stripe**, WP.org-assigned slug `payment-order-reconciler-for-stripe`. "Stripe" trailing was not flagged by the reviewer, but given two prior naming misses, WooCommerce is dropped from the name entirely rather than risk a fourth attempt. Corrected again 2026-09-18 (fourth pass, human review round 1): the assigned slug was accepted, but the actual review team's AI-assisted first pass flagged "Payment Order Reconciler" itself as too generic — it doesn't lead with anything distinctive, even though "for Stripe" is correctly structured. Final public name: **Driftwatch Order Reconciler for Stripe**, requested slug `driftwatch-order-reconciler-for-stripe` — "Driftwatch" is a coined term (not a trademark, not the developer's username) evoking the plugin's actual function. Per WP.org's own review email, a slug change requires an explicit request in the reply, not just a code change; the plugin folder/main file/GitHub repo were deliberately left as `payment-order-reconciler` this round to limit churn — only the Text Domain, i18n calls, and readme.txt title changed.
