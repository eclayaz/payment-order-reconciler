...
- Slug: `payment-order-reconciler` (final, unchanged since 2026-09-12 — was `woo-stripe-reconcile` during development, then `stripe-order-reconciler` at WP.org rollout phase 1. It never contained "woocommerce"; only the display name did.) Public name: **Payment Order Reconciler for Stripe** (was "...for WooCommerce and Stripe" until the actual WP.org review team rejected that submission — their policy bans "woocommerce" from the name/slug outright, in any position, not merely from leading it). Hard dependency on WooCommerce + gateway plugin active. HPOS compatibility declared via `FeaturesUtil::declare_compatibility`. Baseline: WordPress 6.8, PHP 7.4 (the gateway's own requirement, since the target population is gateway users specifically).
