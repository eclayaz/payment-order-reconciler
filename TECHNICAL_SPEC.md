...
- Plugin folder/main file/Plugin URI/GitHub repo: still `payment-order-reconciler` — deliberately not renamed again (see PROBLEM.md's fourth naming pass). WP.org-assigned slug: `payment-order-reconciler-for-stripe`, superseded by a requested-but-not-yet-reserved slug `driftwatch-order-reconciler-for-stripe` (Text Domain header and i18n calls already updated to this; WP.org's own review email says a Text Domain ahead of the reserved slug is expected and fine mid-review). Public name: **Driftwatch Order Reconciler for Stripe** (was "...for Stripe" alone until the review team's AI-assisted pass flagged it as too generic — no distinctive leading term — and before that "...for WooCommerce and Stripe" until rejected outright for containing "woocommerce" at all). Hard dependency on WooCommerce + gateway plugin active. HPOS compatibility declared via `FeaturesUtil::declare_compatibility`. Baseline: WordPress 6.8, PHP 7.4 (the gateway's own requirement, since the target population is gateway users specifically).
