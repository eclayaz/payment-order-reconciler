...
- **Restricted-key scopes needed for v1: seven**, not six. `PaymentIntents:Read`, `Charges:Read`, `Checkout Sessions:Read`, `Events:Read`, `Webhook Endpoints:Read`, `Disputes:Read` (needed for the corrected live dispute-status check in #6/#7), plus **`Reviews:Read`** — found missing post-release (2026-08-30) when a real Pass A run against a live sandbox key scoped to exactly the other six failed outright: the PaymentIntents list call's `expand: data.latest_charge.review` (needed for the live Radar-review downgrade in #6/#7) requires this separate permission, and `WSR_Stripe_Client::check_scopes()` never actually tested for it — so the six-scope "confirmation" the prior revision cited was itself incomplete, checking only what it checked. `check_scopes()` now includes `Reviews:Read`.
