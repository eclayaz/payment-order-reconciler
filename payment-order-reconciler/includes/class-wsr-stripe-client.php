	...
	public function get( $path, array $query = array() ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			$message     = isset( $body['error']['message'] ) ? $body['error']['message'] : sprintf(
				...
				__( 'Stripe API returned HTTP %d.', 'payment-order-reconciler-for-stripe' ),
				$code
			);
		...
		if ( ! is_array( $body ) ) {
			return new WP_Error( 'wsr_stripe_invalid_response', __( 'Stripe API returned an unexpected response.', 'payment-order-reconciler-for-stripe' ) );
		}
