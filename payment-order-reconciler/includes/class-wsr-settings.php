...
class WSR_Settings {
	...
	public static function register_menu() {
		add_submenu_page(
			'woocommerce',
			__( 'Payment Reconciler', 'driftwatch-order-reconciler-for-stripe' ),
			__( 'Payment Reconciler', 'driftwatch-order-reconciler-for-stripe' ),
			self::CAPABILITY,
			'wsr-settings',
	...
	public static function environment_warning() {
		...
		if ( ! $is_test_key && $looks_dev ) {
			return __( 'This looks like a non-production site, but a LIVE restricted key (rk_live_...) is configured. Double-check that\'s intentional — reconciliation checks would run against real payment data.', 'driftwatch-order-reconciler-for-stripe' );
		}

		if ( $is_test_key && 'production' === $env_type && ! $looks_dev ) {
			return __( 'This looks like a production site, but a TEST restricted key (rk_test_...) is configured — reconciliation checks will run against sandbox data, not real payments, until a live key is set.', 'driftwatch-order-reconciler-for-stripe' );
		}
	...
	public static function render_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Payment Reconciler — Settings', 'driftwatch-order-reconciler-for-stripe' ); ?></h1>
				...
				<p>
					<em>
						...
						printf(
							...
							esc_html__( 'This key is set via the %s constant in wp-config.php and cannot be changed here.', 'driftwatch-order-reconciler-for-stripe' ),
			...
			<?php else : ?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<table class="form-table">
						<tr>
							<th scope="row">
								<label for="wsr_api_key"><?php esc_html_e( 'Restricted API key', 'driftwatch-order-reconciler-for-stripe' ); ?></label>
							</th>
							<td>
								<input
									type="password"
								...
								/>
								<p class="description">
									<?php esc_html_e( 'Leave blank and save to remove the currently configured key.', 'driftwatch-order-reconciler-for-stripe' ); ?>
								</p>
							</td>
						</tr>
					</table>
					<?php submit_button( __( 'Save & Validate', 'driftwatch-order-reconciler-for-stripe' ) ); ?>
				</form>
			<?php endif; ?>

			<?php if ( ! empty( $scope_status ) ) : ?>
				<h2><?php esc_html_e( 'Scope check results', 'driftwatch-order-reconciler-for-stripe' ); ?></h2>
				<table class="widefat" style="max-width: 640px;">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Scope', 'driftwatch-order-reconciler-for-stripe' ); ?></th>
							<th><?php esc_html_e( 'Status', 'driftwatch-order-reconciler-for-stripe' ); ?></th>
						</tr>
					</thead>
					<tbody>
					<?php foreach ( $scope_status as $scope => $result ) : ?>
						<tr>
							...
							<td>
								<?php if ( true === $result ) : ?>
									<span style="color:#1a7f37;">&#10003; <?php esc_html_e( 'OK', 'driftwatch-order-reconciler-for-stripe' ); ?></span>
								<?php else : ?>
				...
				<p class="description">
					<?php esc_html_e( 'If any scope failed, edit the restricted key in your Stripe Dashboard (Developers → API keys) to add the missing permission, then save again here.', 'driftwatch-order-reconciler-for-stripe' ); ?>
				</p>
			<?php endif; ?>

			<?php if ( $current_key ) : ?>
				<hr />
				<h2><?php esc_html_e( 'Alerts', 'driftwatch-order-reconciler-for-stripe' ); ?></h2>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					<?php wp_nonce_field( self::NONCE_ACTION_ALERT_EMAIL ); ?>
					<input type="hidden" name="action" value="wsr_save_alert_email" />
					<table class="form-table">
						<tr>
							<th scope="row"><label for="wsr_alert_email"><?php esc_html_e( 'Alert email', 'driftwatch-order-reconciler-for-stripe' ); ?></label></th>
							<td>
								...
								<p class="description"><?php esc_html_e( 'Sent one email per run when new high/critical-severity drift is found. Leave blank to use the site admin email.', 'driftwatch-order-reconciler-for-stripe' ); ?></p>
							</td>
						</tr>
					</table>
					<?php submit_button( __( 'Save', 'driftwatch-order-reconciler-for-stripe' ), 'secondary' ); ?>
				</form>

				<hr />
				<?php self::render_coverage_status(); ?>

				<h2><?php esc_html_e( 'Manual reconciliation', 'driftwatch-order-reconciler-for-stripe' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'Runs the same three steps the daily scheduled job runs: Pass A (reconciliation), Pass B (undelivered-webhook diagnostic), and the webhook endpoint health-check — useful right after saving a key, or any time you don\'t want to wait for the next scheduled run. See the Drift Log dashboard for results.', 'driftwatch-order-reconciler-for-stripe' ); ?>
				</p>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					<?php wp_nonce_field( self::NONCE_ACTION_RUN ); ?>
					<input type="hidden" name="action" value="wsr_run_pass_a" />
					<?php submit_button( __( 'Run reconciliation now', 'driftwatch-order-reconciler-for-stripe' ), 'secondary' ); ?>
				</form>
	...
	public static function render_coverage_status() {
		...
		echo '<h3>' . esc_html__( 'Coverage', 'driftwatch-order-reconciler-for-stripe' ) . '</h3>';
		...
		if ( $failure_is_newer ) {
			...
			printf(
				...
				esc_html__( 'Last run attempt failed %1$s ago: %2$s', 'driftwatch-order-reconciler-for-stripe' ),
				esc_html( human_time_diff( strtotime( $last_run_failure['at'] . ' UTC' ), time() ) ),
				esc_html( $last_run_failure['error'] )
			);
			echo '</p>';
		} elseif ( '' === $last_run_at ) {
			echo '<p><span style="color:#b32d2e;">&#9679;</span> ' . esc_html__( 'No reconciliation run has completed yet.', 'driftwatch-order-reconciler-for-stripe' ) . '</p>';
		} else {
			...
			printf(
				...
				esc_html__( 'Last successful run: %s ago.', 'driftwatch-order-reconciler-for-stripe' ),
		...
		if ( isset( $webhook_health['error'] ) ) {
			echo '<p><span style="color:#b32d2e;">&#9679;</span> ' . esc_html__( 'Webhook health check failed: ', 'driftwatch-order-reconciler-for-stripe' ) . esc_html( $webhook_health['error'] ) . '</p>';
		} elseif ( ! empty( $webhook_health ) ) {
			if ( ! empty( $webhook_health['no_endpoints_configured'] ) ) {
				echo '<p><span style="color:#b32d2e;">&#9679;</span> ' . esc_html__( 'No enabled webhook endpoints found in Stripe at all — this store cannot receive any payment notifications.', 'driftwatch-order-reconciler-for-stripe' ) . '</p>';
			} elseif ( empty( $webhook_health['matching_endpoint_found'] ) ) {
				echo '<p><span style="color:#b32d2e;">&#9679;</span> ' . esc_html__( 'None of the enabled webhook endpoints in Stripe point at this site\'s URL — likely the stale-endpoint problem this plugin exists to catch. Mismatched URLs: ', 'driftwatch-order-reconciler-for-stripe' )
					. esc_html( implode( ', ', $webhook_health['mismatched_endpoint_urls'] ) ) . '</p>';
			} else {
				echo '<p><span style="color:#1a7f37;">&#9679;</span> ' . esc_html__( 'A webhook endpoint matching this site was found and is enabled.', 'driftwatch-order-reconciler-for-stripe' ) . '</p>';
			}
		}

		if ( isset( $pass_b['error'] ) ) {
			echo '<p><span style="color:#b32d2e;">&#9679;</span> ' . esc_html__( 'Undelivered-events check failed: ', 'driftwatch-order-reconciler-for-stripe' ) . esc_html( $pass_b['error'] ) . '</p>';
			...
			printf(
				...
				esc_html__( '%d event(s) failed webhook delivery in the last 30 days.', 'driftwatch-order-reconciler-for-stripe' ),
	...
	private static function render_pass_a_result() {
		...
		if ( is_wp_error( $pass_a ) ) {
			echo '<div class="notice notice-error inline"><p>' . esc_html__( 'Pass A failed: ', 'driftwatch-order-reconciler-for-stripe' ) . esc_html( $pass_a->get_error_message() ) . '</p></div>';
			return;
		}

		$labels = array(
			'payment_intents_scanned'   => __( 'PaymentIntents scanned', 'driftwatch-order-reconciler-for-stripe' ),
			'resolved_to_order'         => __( 'Resolved to a local order', 'driftwatch-order-reconciler-for-stripe' ),
			'no_drift'                  => __( 'Resolved, no drift found', 'driftwatch-order-reconciler-for-stripe' ),
			'stuck_pending_flagged'     => __( 'Stuck-pending flagged', 'driftwatch-order-reconciler-for-stripe' ),
			'wrongly_cancelled_flagged' => __( 'Wrongly-cancelled-paid flagged', 'driftwatch-order-reconciler-for-stripe' ),
			'needs_review'              => __( 'Needs review (unresolved)', 'driftwatch-order-reconciler-for-stripe' ),
			'orphaned_charge_escalated' => __( 'Escalated to orphaned charge', 'driftwatch-order-reconciler-for-stripe' ),
			'skipped_other_site'        => __( 'Skipped (belongs to another site)', 'driftwatch-order-reconciler-for-stripe' ),
			'skipped_too_fresh'         => __( 'Skipped (too fresh, revisit next run)', 'driftwatch-order-reconciler-for-stripe' ),
			'pages_fetched'             => __( 'API pages fetched', 'driftwatch-order-reconciler-for-stripe' ),
		);

		echo '<h3>' . esc_html__( 'Last run results — Pass A', 'driftwatch-order-reconciler-for-stripe' ) . '</h3>';
	...
	private static function render_notice( $notice ) {
		$messages = array(
			'saved'              => array( 'success', __( 'Key saved. See scope check results below.', 'driftwatch-order-reconciler-for-stripe' ) ),
			'cleared'            => array( 'success', __( 'Key removed.', 'driftwatch-order-reconciler-for-stripe' ) ),
			'not_restricted_key' => array(
				'error',
				__( 'That doesn\'t look like a restricted key (should start with rk_live_ or rk_test_). A regular secret key (sk_...) has full write access and is not supported by this plugin — create a restricted, read-only key instead.', 'driftwatch-order-reconciler-for-stripe' ),
			),
			'constant_locked'    => array( 'error', __( 'The API key is locked via wp-config.php and cannot be changed from this screen.', 'driftwatch-order-reconciler-for-stripe' ) ),
			'pass_a_ran'         => array( 'success', __( 'Reconciliation run complete. See results below.', 'driftwatch-order-reconciler-for-stripe' ) ),
			'pass_a_error'       => array( 'error', self::get_pass_a_error_message() ),
			'alert_email_saved' => array( 'success', __( 'Alert email saved.', 'driftwatch-order-reconciler-for-stripe' ) ),
		);
	...
	private static function get_pass_a_error_message() {
		...
		return $message ? sprintf(
			...
			__( 'Reconciliation run failed: %s', 'driftwatch-order-reconciler-for-stripe' ),
			$message
		) : __( 'Reconciliation run failed.', 'driftwatch-order-reconciler-for-stripe' );
	}
}
